...
GLUE_PLUGINS=$(notdir $(wildcard pulp-glue-maven/src/pulp_glue/*))
...
_test: | tests/cli.toml
	pytest -v tests pulp-glue-maven/tests
...
_livetest: | tests/cli.toml
	pytest -v tests pulp-glue-maven/tests -m "$(PYTEST_MARK)"
...
_paralleltest: | tests/cli.toml
	pytest -v tests pulp-glue-maven/tests -m "$(PYTEST_MARK)" -n 8
...
.PHONY: _unittest
_unittest:
	pytest -v tests pulp-glue-maven/tests -m "not live"
...
_unittest_glue:
	pytest -v pulp-glue-maven/tests -m "not live"
